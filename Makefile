...
.PHONY: build
build: ## Build Configs
	ansible-playbook avd/playbooks/build.yml -i avd/inventory.yml -e "target_hosts=CAMPUS_FABRIC"

.PHONY: deploy
deploy: ## Deploy Configs via eAPI
	ansible-playbook avd/playbooks/deploy.yml -i avd/inventory.yml -e "target_hosts=CAMPUS_FABRIC"

.PHONY: validate
validate: ## Validate Fabric
	ansible-playbook avd/playbooks/validate.yml -i avd/inventory.yml -e "target_hosts=CAMPUS_FABRIC"
